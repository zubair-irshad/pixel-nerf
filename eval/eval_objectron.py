...
args, conf = util.args.parse_args(
    extra_args, default_conf="conf/exp/objectron.conf", default_expname="shapenet",
)
args.resume = True
...
print("len(args.viewlist)", len(args.viewlist), args.viewlist)
    ...
    for obj_idx, data in enumerate(data_loader):
        ...
        if not args.no_compare_gt:
            ...
            for view_idx in range(n_gen_views):
                ...
                print("rgb_gt_all, masks_gt_all, all_rgb", rgb_gt_all[view_idx].shape, masks_gt_all[view_idx].shape, all_rgb[view_idx].shape)
                masked_gt_rgb = np.ones(rgb_gt_all[view_idx].shape)
                masked_rgb = np.ones(rgb_gt_all[view_idx].shape)

                print("masked_gt_rgb, maskedrgb", masked_gt_rgb.shape, masked_rgb.shape)
                masks_gt_single = masks_gt_all[view_idx].squeeze(-1)
                print("masks_gt", masks_gt_single.shape)
                print("all_rgb[view_idx]", all_rgb[view_idx].shape,masks_gt_single.shape )
                masked_rgb[masks_gt_single] = all_rgb[view_idx][masks_gt_single]
                masked_gt_rgb[masks_gt_single] = rgb_gt_all[view_idx][masks_gt_single]
        
                print("masked_rgb, masked_gt_rgb", masked_rgb.shape, masked_gt_rgb.shape)
                ssim_obj = skimage.measure.compare_ssim(
                    masked_rgb,
                    masked_gt_rgb,
                    multichannel=True,
                    data_range=1,
                )
                psnr_obj = skimage.measure.compare_psnr(
                    masked_rgb, masked_gt_rgb, data_range=1
                )
                ...
                curr_ssim_obj += ssim_obj
                curr_psnr_obj += psnr_obj
        ...
        curr_psnr_obj /= n_gen_views
        curr_ssim_obj /= n_gen_views
        curr_cnt_obj = 1
        total_psnr_obj += curr_psnr_obj
        total_ssim_obj += curr_ssim_obj
        cnt_obj += curr_cnt_obj
        if not args.no_compare_gt:
            print(
                "curr psnr_obj",
                curr_psnr_obj,
                ...
                curr_ssim_obj,
                "running psnr_obj",
                total_psnr_obj / cnt_obj,
                "running ssim_obj",
                total_ssim_obj / cnt_obj,
            )
        finish_file.write(
